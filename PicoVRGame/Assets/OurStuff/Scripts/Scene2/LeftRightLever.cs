...
public class LeftRightLever : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        float currentTilt = transform.localRotation.eulerAngles.z - startRotation;

        if (currentTilt <= 350 && currentTilt >= 300)
        {
            float tiltStrength = Mathf.Abs(currentTilt - 360);
            leftRightTilt = -tiltStrength;
            isDriving = true;
            Debug.Log("LeftSpeed" + leftRightTilt);
        }

        else if (currentTilt <= 60 && currentTilt >= 10)
        {
            leftRightTilt = currentTilt;
            isDriving = true;
            Debug.Log("RightSpeed" + leftRightTilt);
        }

        else if(currentTilt > 60 && currentTilt < 300 || currentTilt < 10 && currentTilt >= 0 || currentTilt > 350 && currentTilt <= 360)
        {
            leftRightTilt = 0;
            isDriving = false;
            Debug.Log("TiltMore");
    ...
    private void CalculateRotation(Vector3 directionForward, Vector3 directionUpward)
    {
        ...
        if (rotation.z >= 0 && rotation.z <= 60 || rotation.z >= 300 && rotation.z <= 360)
        {
